	...
	<div class="__wrapper c-main">
		<div class="grid grid-cols-1 md:grid-cols-[4fr_6fr]  items-start gap-10 z-10 relative">
			...
			<div class="__content l py-0 lg:sticky lg:top-36 z-20 ">
				...
				<h2 data-gsap-element="title" class="text-h2 text-primary mb-6 ">{{ $g_duotitle['title'] }}</h2>
				@endif
				@if(!empty($g_duotitle['image']['url']))
				<div data-gsap-element="" imgclass="overflow-visible w-10 h-10 max-h-[300px]">
					<img class=" object-cover max-h-80 w-full rounded-2xl" src="{{ $g_duotitle['image']['url'] }}" alt="{{ $g_duotitle['image']['alt'] ?? '' }}" loading="lazy" decoding="async">
				</div>
				@endif
				...
				<a data-gsap-element="btn" class="btn-primary btn m-btn second-btn mt-6 inline-block" href="{{ $g_duotitle['button']['url'] }}">{{ $g_duotitle['button']['title'] }}</a>
				@endif
			</div>
			...
			<div class=" w-full z-10 relative lg:pt-0">
				...
				<div class="__cards-repeater flex flex-col gap-5 w-full h-full relative z-10 ">
					...
					<div data-gsap-element="stagger"  class="group block no-underline ">
						<div class="grid grid-cols-1  items-stretch bg-white transition-all duration-300 rounded-3xl   h-full">
